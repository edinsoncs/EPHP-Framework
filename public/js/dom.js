                            ...
                            '<header class="header text-center pt-2">'+
                               '<h3 class="m-0 font-weight-light">'+data.name+'</h3>'+
                                '<div class="flag text-center">'+
                                    '<span class="text-muted small">'+data.citi+'</span>'+
                                '</div>'+
                            '</header>'+
                        '</div>';
        _article.innerHTML = person;
...
function preview_image(event) 
{
	 ...
	 {
	      var image_p = getCss('js-image');
		  removeClass(image_p, 'none');
...
function activeComment(t){

	var pa = $(t).parent().parent().parent();
	$(pa).find('js-comments-opt').css({'display': 'block'});
	console.log(pa);	
}
